...
def init(request):
    ...
    if 'username' not in body:
        return JsonResponse(status=400, data={'message':'Username is required to join channel'})
    ...
    try:
        ...
        token = bytes(client.create_token(user_id=member.username), "utf-8").decode("utf-8")
